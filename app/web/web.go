...
import (
	...
	"github.com/zkeai/cusdt/app/help"
	"github.com/zkeai/cusdt/app/log"
	"github.com/zkeai/cusdt/static"
	"html/template"
	"io/fs"
	"net/http"
)
...
func Start() {
	...
	r.GET("/", func(c *gin.Context) {
		c.HTML(200, "index.html", gin.H{
			...
			"url":   "https://github.com/zkeai/cusdt",
		})
	})
	...
	orderRoute := r.Group("/api/v1/order")
	{
		orderRoute.Use(func(ctx *gin.Context) {
			...
			if help.GenerateSignature(m, config.GetAuthToken()) != sign {
				log.Warn("签名错误", m)
				ctx.JSON(400, gin.H{"error": help.GenerateSignature(m, config.GetAuthToken())})
				ctx.Abort()
			}
